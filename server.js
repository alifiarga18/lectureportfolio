...
const server = http.createServer((req, res) => {
    ...
    // ── Save site-config.json (called after installation) ──
    if (req.url === '/api/save-site-config' && req.method === 'POST') {
        let body = '';
        req.on('data', chunk => body += chunk);
        req.on('end', () => {
            try {
                ...
                const safeConfig = {
                    ...
                    installed: true,
                    installed_at: configData.installed_at || new Date().toISOString()
                };
